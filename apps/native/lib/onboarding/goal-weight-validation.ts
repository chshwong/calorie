  ...
  switch (goalType) {
    case "lose": {
      ...
      const safeRecommend =
        ...
        recommendedLb >= PROFILES.WEIGHT_LB.MIN &&
        recommendedLb <= PROFILES.WEIGHT_LB.MAX;
      return {
        minLb: allowedMin,
        maxLb: allowedMax,
    ...
    case "maintain": {
      ...
      const recommendedLb = currentWeightLb;
      const safeRecommend =
        allowedMin <= allowedMax &&
        recommendedLb >= allowedMin &&
        recommendedLb <= allowedMax &&
    ...
    default:
      return {
        minLb: PROFILES.WEIGHT_LB.MIN,
        maxLb: PROFILES.WEIGHT_LB.MAX,
  ...
  if (targetLb < PROFILES.WEIGHT_LB.MIN || targetLb > PROFILES.WEIGHT_LB.MAX) {
    if (weightUnit === "kg") {
      return {
        ...
        i18nParams: {
          minKg: roundTo1(DERIVED.WEIGHT_KG.MIN),
          maxKg: roundTo1(DERIVED.WEIGHT_KG.MAX),
        },
      };
    }
    return {
